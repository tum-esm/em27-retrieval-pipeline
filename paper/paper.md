...
affiliations:
  ...
  - name: Karlsruhe Institute of Technology (KIT), Karlsruhe, Germany
    index: 2
  - name: Finnish Meteorological Institute, Sodankylä, Finland
    index: 3
...
- **Configuration:** JSON Configuration files are used to specify where to find the input data, which tasks to perform, which arguments to use during the retrieval, and so on. These files are validated against JSON schemas [@jsonschema;@pydantic] representing a strict set of rules of how a configuration file can look. By this, a misconfiguration will be detected immediately, as opposed to only failing when the code tries to access invalid parts of the configuration. The JSON schema is rendered as an API reference in the documentation, enabling us to manage and adequately document a large number of configuration parameters.
- **Metadata Management:** Metadata (where/when/how each sensor was set up) is provided as JSON files – again, validated against a documented JSON schema. These files can be stored locally or pulled from a GitHub repository, enabling centralized, version-controlled tracking of sensor deployments. We believe all metadata systems used in different EM27/SUN teams can be easily translated into the schema we established with this pipeline. Our metadata schema design largely agrees with the schema proposed in @Zeeman2024.
- **Parallelization and Containerization:** While the PROFFASTpylot parallelizes the individual steps of PROFFAST, the pipeline parallelizes the whole retrieval process by running one isolated retrieval job per sensor per day. Every result folder maintains the structure of an individual PROFFASTpylot run, but also stores all configuration files required to fully reproduce the output in this folder.
...
- **Caching/Implicit Scheduling:** Users do not have to define an explicit queue of jobs, but instead define a date range and a list of sensors to include. The downloading/retrieval/bundling will then decide which jobs to run. I.e., the pipeline only requests atmospheric profiles that have not been downloaded yet, and it only runs retrievals for data with no existing retrieval result (for the same algorithm and atmospheric profile).
- **Testing:** The codebase is fully statically typed and checked with strict MyPy [@mypy], and includes unit- and end-to-end tests (using PyTest [@pytest]) covering all major functions and retrieval scenarios.

Furthermore, the pipeline adds features required to produce long-term EM27/SUN datasets:

- **A fully automated interface to obtain Ginput data:** The atmospheric profiles downloader automates requests for atmospheric profiles in the GGG2014 or GGG2020 format from a Ginput server [@Laughner2021;@Laughner2023;@tcconwiki].
- **Bundling of retrieval results:** The raw retrieval outputs are distributed over hundreds or thousands of folders. The bundling script of this pipeline merges these outputs into one file per sensor, retrieval algorithm, and atmospheric profile, which is significantly more suitable for distribution.

\autoref{fig:architecture} shows the current building blocks of the EM27 Retrieval Pipeline. For the full feature set of the pipeline, please refer to its GitHub repository at [github.com/tum-esm/em27-retrieval-pipeline](https://github.com/tum-esm/em27-retrieval-pipeline) and documentation. For example, starting with version 1.8 of the pipeline, the outputs include selected parameters extracted from the OPUS files, like the scanner temperature or laser wavenumber, parsed with the OPUS-file reader from the `tum-esm-utils` library [@tumesmutils]. Version 1.9 of the pipeline introduced an event flagging system to mark and quality-flag operational events in the output bundles.
...
The EM27/SUN is widely used to achieve these GHG flux estimations [@Hase2015;@Klappenbach2015;@Chen2016;@Butz2017;@Vogel2019;@Luther2019;@Dietrich2021;@Jones2021;@Luther2022;@Tu2022;@Forstmaier2023;@Doc2025journal;@Stauber2025]. Unlike the HR125 used by TCCON [@tccon], the EM27/SUN can be transported and deployed at different – even very remote – locations with much effort. Our EM27/SUN systems are not only used in the permanent urban sensor network MUCCnet, but also in temporary measurement campaigns. Since 2016, the instruments of MUCCnet have been deployed at 3 locations in San Francisco [@Klappenbach2021], 4 in Hamburg [@Forstmaier2023], 4 in Vienna [@Luther2023], 12 in Poland [@Luther2019;@Luther2022], and 11 locations in Munich [@Chen2018;@Dietrich2021].

\autoref{table:coverage} shows the data output of our automated setup of MUCCNet [@Heinle2018;@Dietrich2021;@Aigner2023] in 2024. \autoref{fig:data} shows the XCO2 timeseries of the MUCCnet instruments since September 2019. Running the retrievals for various instrument deployments and keeping track of all deployments over time requires an organizational system that this pipeline provides.

| **Sensor**     | **Measurements** | **Days with at least 3h Coverage** | **Avg. Daily Coverage** |
| :------------- | :--------------- | :------------------------------ | :---------------------- |
| **ma (SN061)** | 368,643          | 157                             | 4h 38m                  |
| **mb (SN086)** | 322,804          | 141                             | 4h 27m                  |
| **mc (SN115)** | 428,582          | 160                             | 4h 41m                  |
| **md (SN116)** | 416,638          | 151                             | 4h 32m                  |
| **me (SN117)** | 431,110          | 130                             | 3h 53m                  |

Table: Measurement coverage of MUCCnet in 2024. \label{table:coverage}

To enable a large number of long-term observations with EM27/SUN spectrometers, such as the networks of COCCON-Spain [@cocconspain] and GEMINI-UK [@geminiuk], an improvement in automation is crucial. The EM27/SUN Retrieval Pipeline addresses this need by significantly reducing the data processing and housekeeping workload compared to previous workflows.
...
The pipeline was first established in 2021 [@Rimann2022] and has been used for our entire EM27/SUN dataset since 2022 [@Luther2023;@Klappenbach2024;@Chen2024;@Tang2024;@Loew2025;@Stauber2025]. Multiple journal publications using MUCCnet for inverse modeling are currently in preparation. The data generated by MUCCnet and the EM27 Retrieval Pipeline has been published to the ICOS Carbon Portal [@Makowski2023CarbonPortal;@Makowski2024CarbonPortal] and the EVDC Portal [@Loew2025EVDC;@Chen2025EVDCOCOPF10;@Chen2025EVDCOCOPF22] and is an important data input for the GHG modeling approaches developed with the grants listed below.
