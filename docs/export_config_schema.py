...
MD_FILE_TARGET = os.path.join(
    PROJECT_DIR, "docs", "pages", "docs", "api-reference", "configuration-files.mdx"
)
...
example_file_blocks = re.findall(r"Example File\n\n```[\s\S]*?```", md_file_content)
...
with open(os.path.join(PROJECT_DIR, "config", "manual-queue.template.json")) as f:
