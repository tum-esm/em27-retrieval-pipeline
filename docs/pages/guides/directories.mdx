...
Each column is named like this: `<sensorid>__<location_id>__<datatype>`. The allowed data types are listed in the [config schema](/api-reference/configuration).
...
The containers where the retrieval is actually running, are working on `data/containers`. Each container with a container name like `eloquent-oppenheimer` has three active directories: `data/containers/retrieval-container-$containername`, `data/containers/retrieval-container-$containername-input`, and `data/containers/retrieval-container-$containername-output`.
