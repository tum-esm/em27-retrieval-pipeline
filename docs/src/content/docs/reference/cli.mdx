...
be found at `$ERP_LOGS_DIR/retrieval` (default: `data/logs/retrieval`).
...
Run the retrieval. Blocking. The logs and the current processing queue from
this process can be found at `$ERP_LOGS_DIR/retrieval` (default:
`data/logs/retrieval`).
...
current processing queue from this process can be found at
...
queue from this process can be found at `$ERP_LOGS_DIR/retrieval` (default:
`data/logs/retrieval`).
...
`python cli.py ggg-profiles-downloader run [OPTIONS]`
...
`python cli.py ggg-profiles-downloader request-ginput-status [OPTIONS]`
...
`python cli.py ggg-profiles-downloader migrate-storage-location [OPTIONS]`
...
`python cli.py fetch-remote-metadata [OPTIONS]`

**Description:**

Fetch the configured GitHub metadata and store it in the config directory.
