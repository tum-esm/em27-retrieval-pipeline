...
You can specify multiple entries in the bundle exports config section and the script will run the bundling for all of them.
...
and `geoms_metadata.json` to be present in the configuration directory. You can use the [example files and the API reference](/reference/geoms-configuration)
to create your own files. You need to add a section `geoms` to the [general `config.json` file ](/reference/configuration) defining
