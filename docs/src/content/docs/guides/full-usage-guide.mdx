...
## Downloading Atmospheric Profiles with the GGG Profiles Downloader

This pipeline automates the process of obtaining atmospheric profiles described in the [Ginput documentation](https://tccon-wiki.caltech.edu/Main/ObtainingGinputData). Configure the FTP email address under `ggg_profiles_downloader.server.email` and the requested date range, models, and locations under `ggg_profiles_downloader.scope`.
...
The downloader uses the metadata and configured local profiles directory to determine which profiles are missing. Each run requests missing profiles and checks the results of ongoing requests.
...
```bash
python cli.py ggg_profiles_downloader request-ginput-status
```
...
You can limit the number of concurrent retrieval processes using `retrieval.general.max_process_count`.
...
<img
  src="/em27-retrieval-pipeline/images/cli-retrieval-watch.png"
...
You can specify multiple `[[bundle_exports]]` entries in `config.toml`; the command processes each configured export.
...
This pipeline can generate GEOMS-compliant HDF5 files from retrieval outputs. Add one or more `[[geoms_exports]]` entries to `config.toml` to define the sensors, algorithms, atmospheric-profile models, time range, and filtering settings to export. The separate `geoms_metadata.toml` file contains the GEOMS file metadata and calibration factors. See the [`config.toml` reference](/em27-retrieval-pipeline/reference/config), [`geoms_metadata.toml` reference](/em27-retrieval-pipeline/reference/geoms-metadata), and repository templates for the complete schemas.
...
The logs report which files were generated:
...
This command produces one CSV file per sensor ID in the `data/reports/` directory. For example:
...
The `interferograms` and `ground_pressure` columns contain the number of interferograms and ground-pressure rows found for the respective day. Retrieval and profile columns use `✅` for present data, `❌` for failed retrievals, and `-` when no data is present.
...
#!/bin/bash
#SBATCH -J erp
#SBATCH -o /path/to/em27-retrieval-pipeline/data/logs/%x.%j.%N.out
#SBATCH -D /path/to/em27-retrieval-pipeline
#SBATCH --clusters=cm4
...
# running pipeline, gfortran (GCC) is used to compile PROFFAST and the IFG
# corruption filter
