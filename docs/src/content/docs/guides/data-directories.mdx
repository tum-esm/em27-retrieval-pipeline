...
The pipeline and none of the retrieval algorithms manipulate any data stored in the directories `config.data.ground_pressure.path`, `config.data.atmospheric_profiles.path`, and `config.data.interferograms.path`. The retrieval only writes to `config.data.results.path`.

<Aside type="note">

We can guarantee that this pipeline doesn't modify any data from the input directories. We cannot guarantee this for all of the underlying retrieval algorithms, but we are very confident about that they also don't manipulate any of the input data.
...
Point the `config.data.interferograms.path` variable to the directory your interferograms are stored in.

<FileSystemTree
  schema={{
    type: "directory",
    title: "<config.data.interferograms.path>",
    items: [
      {
        type: "directory",
        title: "ma",
...
In this example, `ma`, `mb`, and so on are the "sensor ids" used by us (see the [next section about the EM27 metadata](/em27-retrieval-pipeline/guides/em27-metadata)).

You must set the `config.data.interferograms.ifg_file_regex` value to a regex matching your files. In the example above, we can use `^$(SENSOR_ID)$(DATE).*ifg.\\d+$`.

### Ground Pressure Files

Point the `config.data.ground_pressure.path` variable to the directory where you store your local meteorological files, i.e. your ground pressure logs.

<FileSystemTree
  schema={{
    type: "directory",
    title: "<config.data.ground_pressure.path>",
    items: [
      {
        type: "directory",
        title: "ma",
...
- Pressure column: Pass the name of the column with `config.data.ground_pressure.pressure_column` and specify the unit (either `hPa` or `Pa`) with `config.data.ground_pressure.pressure_column_format`.
...
Point the `config.data.atmospheric_profiles.path` variable to the directory you want to store the atmospheric profiles.

<FileSystemTree
  schema={{
    type: "directory",
    title: "<config.data.atmospheric_profiles.path>",
    items: [
      {
        type: "directory",
        title: "GGG2014",
...
<FileSystemTree
  schema={{
    type: "directory",
    title: "<config.data.results.path>",
    items: [
      {
        type: "directory",
        title: "proffast-2.3",
                        ...
                        items: [
                          {
                            ...
                            items: [
                              {
                                type: "file",
                                title: "invers20ma_210101_a.inp",
                              },
                              { type: "file", title: "pcxs20ma_210101.inp" },
                              {
                                type: "file",
                                title: "preprocess5ma_210101.inp",
                              },
                            ],
                          },
                          {
                          ...
                          {
                            type: "file",
                            title: "comb_invparms_ma_SN061_210101-210101.csv",
                          },
...
The output files will include all data from the `<config.data.results.path>` directory that matches the time period. The CSV and Parquet files contain the same data - just in two different tabular formats. They keep all columns from the raw retrieval algorithm but add four more columns `utc`, `retrieval_time`, `location_id` and `campaign_ids`:
...
The logs are stored in `data/logs` by default. Set `ERP_LOGS_DIR` to use another existing base directory; the pipeline creates its `retrieval` and `retrieval/archive` directories automatically:
    ...
    items: [
      {
        ...
        items: [
          {
            type: "file",
            title: "20240106-23-54_main.log",
            description: "Log file a fill retrieval run.",
          },
          {
            type: "file",
            title: "20240106-22-55_generous-easley.log",
            description: "Log file of an individual container.",
          },
          {
            type: "file",
            title: "20240106-23-10_eloquent-oppenheimer.log",
            description: "Log file of an individual container.",
          },
          {
            type: "directory",
            ...
            items: [
              {
                type: "directory",
                title: "container",
                description: "old container logs",
              },
              {
                type: "directory",
                title: "main",
                description: "old main logs",
              },
            ],
          },
        ],
