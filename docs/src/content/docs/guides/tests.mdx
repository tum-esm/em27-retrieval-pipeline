...
When running the `ci` or `complete` tests, the test suite will download some sample data to `data/testing/container/inputs`. After the tests have run successfully, the directory `data/testing/container/outputs` will be fully populated with the results of any retrieval algorithm using any atmospheric profiles.
