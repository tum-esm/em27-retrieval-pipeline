...
The logs are stored in `data/logs` by default. Set `ERP_LOGS_DIR` to use another
existing base directory; the pipeline creates its `retrieval` and
`retrieval/archive` directories automatically:
