...
def _generate_pylot_config(session: custom_types.ProffastSession) -> None:
    ...
    tum_esm_utils.files.dump_file(
        ...
        tum_esm_utils.text.insert_replacements(
            file_content,
            {
                ...
                "COORDINATES_LAT": str(round(session.ctx.location.lat, 3)),
                "COORDINATES_LON": str(round(session.ctx.location.lon, 3)),
                "COORDINATES_ALT": str(round(session.ctx.location.alt / 1000.0, 3)),
                "UTC_OFFSET": str(round(session.ctx.utc_offset, 2)),
...
def _generate_pylot_log_format(session: custom_types.ProffastSession) -> None:
    file_content = tum_esm_utils.files.load_file(
        ...
        tum_esm_utils.text.insert_replacements(
            file_content,
            {
                ...
                "PRESSURE_CALIBRATION_FACTOR": str(
                    round(session.ctx.pressure_calibration_factor, 9)
                ),
