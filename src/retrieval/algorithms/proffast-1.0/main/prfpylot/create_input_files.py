...
def _write_template_file(
    ...
    file_content = (
        tum_esm_utils.text.insert_replacements(template_content,
                                               replacements).rstrip("\n ") +
        "\n"
    )
...
def create_pcxs_input_file(session: utils.types.RetrievalSession) -> None:
    ...
    _write_template_file(
        session, src_filepath, dst_filepath, additional_lines=[]
    )
...
def create_invers_input_file(session: utils.types.RetrievalSession) -> None:
    ...
    spectra_filenames = [
        f for f in os.listdir(spectra_dir) if f.endswith(f"SN.BIN")
    ]
    _write_template_file(
        session,
        src_filepath,
...
def move_profiles_and_datalogger_files(
    ...
    profile_src = os.path.join(
        session.ctn.data_input_path, "map", profile_filename
    )
