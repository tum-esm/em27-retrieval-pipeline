...
integer,parameter :: nNLCifg = 12 ! number of ifgs for fitting a detector response curve
integer,parameter :: nNLCradius = 256 ! number of ifg points used for DFT is 2 * nNLCradius + 1
...
logical :: obsfixdec,checkoutdec,quietrundec,dualchandec,chanswapdec,anaphasdec,NLCdec,NLCfitdec
character(len=300) :: infotext,diagoutpath,binoutpath
character(len=30) :: obslocation
integer :: mpowFFT,ifgradius,maxifg,maxspc,maxspcrs,bandselect,nNLCparm,nNLCconfig
integer :: nmeas,nsinc !nsinc: width of sinc kernel on oversampled fft grid -nsinc...+nsinc
real :: obsfixaltkm,ILSapo,ILSapo2,ILSphas,ILSphas2,DCmin,DCvar,toffseth_UT
real(8) :: obsfixlatdeg,obsfixlondeg
real(8) :: OPDmax,semiFOV


end module glob_prepro62
