...
class FileMover(Preparation):
    ...
    def move_results(self):
        ...
        for local_date in self.local_dates:
            ...
            try:
                ...
                self.logger.error("OSError while moving file " f"{sfile}. Errormessage: {e}")
            ...
            for suffix in suffix_list:
                ...
                for sfile in sourcefiles:
                    ...
                    try:
                        ...
                        self.logger.error("OSError while movig file " f"{sfile}. Errormessage: {e}")
    ...
    def delete_abscos_files(self):
        ...
        for date in self.meas_dates:
            ...
            try:
                ...
                self.logger.error("File not Found: " f"Could not delete {filename}")
    ...
    def delete_input_files(self):
        ...
        for inp_file in self.global_inputfile_list:
            try:
                ...
                self.logger.error(
                    "File not Found: " f"Could not remove {type} input file" f" {inp_file}"
                )
    ...
    def move_input_files(self):
        ...
        for inp_file in self.global_inputfile_list:
            try:
                ...
                self.logger.error(
                    "File not found: " f"Could not move {type} input file" f" {inp_file}."
                )
    ...
    def _move_generallogfile_to_logdir(self):
        ...
        target = os.path.join(self.logfile_folder, os.path.basename(self.pylot_log))
        try:
            shutil.move(self.pylot_log, target)
            self.logger.debug("Moved the general logfile to the" " result/logfiles folder.")
        except FileNotFoundError as e:
            self.logger.debug(f"\nsource: {self.pylot_log} " f"\ntarget: {target}")
            self.logger.debug(e)
            self.logger.error(
                "Could not move logfile to new logfile dir! "
    ...
    def _move_prf_config_file(self):
        ...
        self.logger.debug(
            "Copying the PROFFASTpylot input_file " f"{self.input_file} to {self.result_folder}"
        )
