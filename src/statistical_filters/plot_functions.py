        ...
        for keydate, groupdate in df_reset.groupby("Date"):
            ...
            for keysite, groupsite in groupdate.groupby("ID_Spectrometer"):
                groupsite.plot(
                    ...
                    .loc[
                        df_color["ID_Spectrometer"]
                        == groupsite["ID_Spectrometer"].values[0]
                    ]
                    .values[0],
                )
            if show:
...
def plot_heat_scatter(df, x_column, y_column, **kwargs):
    ...
    r_2 = utils.r2(x, y)
    print(
        "The r2 coefficent between {} and {} is: {:f}".format(x_column, y_column, r_2)
    )
...
def plot_bokeh(df, x_column, y_column, **kwargs):
    ...
    r_2 = utils.r2(df[x_column], df[y_column])
    print(
        "The r2 coefficent between {} and {} is: {:f}".format(x_column, y_column, r_2)
    )
...
def plot_regressionPlot(df, x_column, y_column, **kwargs):
    ...
    sns.regplot(
        x=x_column, y=y_column, data=df, ci=0, color="b", line_kws={"color": "red"}
    )
    plt.xlabel(str_x)
    plt.ylabel(str_y)
    plt.title(str_title)
...
def plot_location(df, df_location):
    ...
    df_locationUsed = df_location.loc[
        df_location["ID_Location"].isin(locationID)
    ].copy()
    myProj = pyproj.Proj(init="epsg:3857")
    lon, lat = myProj(
        df_locationUsed["Coordinates_Longitude"].values,
        df_locationUsed["Coordinates_Latitude"].values,
        inverse=False,
    )
    df_locationUsed["Merc_Long"], df_locationUsed["Merc_Lat"] = myProj(
        df_locationUsed["Coordinates_Longitude"].values,
        df_locationUsed["Coordinates_Latitude"].values,
        inverse=False,
    )
    ...
    df_LocSite = (
        df.groupby(["ID_Location", "ID_Spectrometer"])
        .size()
        .reset_index()
        .rename(columns={0: "count"})
    )
    # join the two dataframes such that all nessecary information are located in table
    df_alldata = df_locationUsed.set_index("ID_Location").join(
        df_LocSite.set_index("ID_Location")["ID_Spectrometer"]
    )
    ...
    p = figure(
        x_range=(1220000, 1350000),
        y_range=(6110000, 6150000),
    ...
    tooltips = [
        ("ID_Spectrometer", "@ID_Spectrometer"),
...
def plot_windrose(df, column, title, **kwargs):
    ...
    df_windclu = df_wind.groupby(
        ["windSpd30m", "windDir30m_rad"], as_index=False
    ).mean()

    if compare == "count":
        df_windclu["count"] = df_wind.groupby(
            ["windSpd30m", "windDir30m_rad"], as_index=False
        ).count()["windDir30m"]
        np_windval = df_windclu[
            ["windSpd30m", "windDir30m_rad", column, "count"]
        ].values
    elif compare is not None:
        np_windval = df_windclu[
            ["windSpd30m", "windDir30m_rad", column, compare]
        ].values
    else:
    ...
    if compare is not None:
        ...
        np_sortedcompare[r_index.astype(int), theta_index.astype(int)] = np_windval[
            :, 3
        ]
