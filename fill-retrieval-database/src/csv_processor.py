...
def detect_new(df_from_csv: DataFrame, df_from_cache: DataFrame) -> DataFrame:
    new_rows = (
        pandas.merge(
            ...
            indicator=True,
            how="left",
            on=["utc", "sensor", "retrieval_software"],
            suffixes=("", "_y"),
        )
...
def detect_modified(
    df_from_csv: DataFrame, df_from_cache: DataFrame, new_rows: DataFrame
) -> DataFrame:
    modified_rows = (
        pandas.merge(
            df_from_csv, df_from_cache, indicator=True, how="left", suffixes=("", "_y")
        )
        .query('_merge=="left_only"')
        .drop("_merge", axis=1)
    )
