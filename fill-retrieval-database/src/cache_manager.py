...
class CacheProxy:
    ...
    def load_cache_in_memory_for(self, year: int) -> DataFrame:
        ...
        if os.path.isfile(self.__build_filename_from_year(year)):
            ...
            cached_data_frame = pandas.read_hdf(
                self.__build_filename_from_year(year), key=self.__KEY_VALUE
            )
    ...
    def get_all_years_in_cache(self) -> List[int]:
        if os.path.isdir(self.location):
            ...
            years_in_cache = [
                self.__map_filename_to_year(filename.removeprefix(self.location + "/"))
    ...
    def refresh_contents(self, dataframe: DataFrame, year: int):
        ...
        dataframe.to_hdf(
            self.__build_filename_from_year(year), key=self.__KEY_VALUE, mode="w"
        )
